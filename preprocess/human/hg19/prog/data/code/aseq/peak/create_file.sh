...
for cpath in \$(find \$thome/?? -mindepth 0 -maxdepth 0 -type d | sort); do
	cname=\`basename \$cpath\`

	printf "[%s] Start  check md5sum for \$cname\n" \`date +%T\`
	rm -f \$ttemp/\$cname.md5
	for fpath in \$(find \$cpath/* ! -type d); do
	...
	if [[ ! -e \$cname.md5 ]]; then
		...
		rm -f \$cname.txt
		for fpath in \$(find \$cpath/*.bed.gz ! -type d 2>/dev/null); do
			echo \$fpath >>  \$cname.txt
		done
		for fpath in \$(find \$cpath/*.cnt.gz ! -type d 2>/dev/null); do
			echo \$fpath >>  \$cname.txt
		done
		if [[ -s \$cname.txt ]]; then
			\$phome/create_fbin.exe \`readlink -e \$cname.txt\` \$ttemp/\$cname
			for ppart in \$(find \$ttemp/\$cname??.bin); do
				npart=\`basename \$ppart\`
				cp -a \$ttemp/\$npart \$thome/
			done
	...
	for ppart in \$(find \$thome/\$cname??.bin); do
		npart=\`basename \$ppart\`
		if [ ! -e \$dhome/\$npart ]; then
			rm -f \$dhome/\$npart
			ln -s \$thome/\$npart \$dhome/\$npart
		fi
	done
done
